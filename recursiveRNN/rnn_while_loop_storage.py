...
class RNN_Model():
    ...
    def _loop_over_tree(self, i, tensor_list):
        ...
        tensor_list = tf.concat([tensor_list, node_tensor], 0)
        i = tf.add(i, 1)
    ...
    def train(self, sess, verbose=True):
        ...
        for epoch in range(self.config.max_epochs):
            print('epoch %d' % epoch)
            if epoch == 0:
                train_acc, val_acc, loss_history, val_loss = self.run_epoch(new_model=True, epoch=epoch, sess=sess,
                                                                            verbose=verbose,
                                                                            summary_writer=summary_writer)
            else:
                train_acc, val_acc, loss_history, val_loss = self.run_epoch(epoch=epoch, sess=sess, verbose=verbose,
                                                                            summary_writer=summary_writer)
