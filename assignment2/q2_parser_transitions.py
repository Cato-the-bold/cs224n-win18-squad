class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        """
        if transition=="S":
            token, self.buffer = self.buffer[0], self.buffer[1:]
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        unfinished_parses = unfinished_parses[batch_size:] + [parse for parse in mini_batch if not (len(parse.stack)==1 and len(parse.buffer)==0)]
