...
from util import print_sentence, write_conll, read_conll, window_iterator
...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        mask = [1]*min(max_length, len(sentence)) + [0]*max(0, max_length - len(sentence))
        sentence = sentence[0:max_length]
        sentence += zero_vector*(max_length - len(sentence))
        labels = labels[0:max_length]
        labels += [zero_label]*(max_length - len(labels))
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(embed, self.input_placeholder), shape=[-1,Config.max_length, Config.n_features*Config.embed_size])
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        U = tf.Variable(xavier_initializer([Config.hidden_size, Config.n_classes]), dtype=tf.float32, name="U")
        b2 = tf.Variable(tf.zeros([1, Config.n_classes]), dtype=tf.float32, name="b2")
        s = tf.shape(X)[1:] #batch * ebed
        h = tf.zeros(s)
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                if time_step>0:
                    ...
                    output, state = cell(data, state)
                else:
                    output, state = cell(data, h)
