...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self):
        ...
        _, state = tf.nn.dynamic_rnn(cell, self.inputs_placeholder,
                                           initial_state=tf.zeros_like(self.labels_placeholder),
                                           dtype=tf.float32)
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.l2_loss(tf.subtract(preds, y))
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients:
            gradients, norm = tf.clip_by_global_norm(gradients, self.config.max_grad_norm)
            train_op = optimizer.apply_gradients(zip(gradients, params))
        else:
            train_op = optimizer.minimize(loss)
            norm = tf.global_norm(params)
