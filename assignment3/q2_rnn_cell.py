...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            W_x = tf.get_variable("W_x", shape=[self.input_size, self.state_size], dtype=tf.float32, initializer=initializer)
            W_h = tf.get_variable("W_h", shape=[self.state_size, self.state_size], dtype=tf.float32, initializer=initializer)
            b = tf.get_variable("b", shape=[self.state_size], dtype=tf.float32, initializer=initializer)

            new_state = tf.sigmoid(tf.matmul(inputs, W_x) + tf.matmul(state, W_h) + tf.reshape(b, [1, self.state_size]))
