...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        sentence0 = [start]+sentence+[end]
        for i in range(window_size, len(sentence)+window_size):
            window = sentence0[i - window_size:i + window_size + 1]
            flatten = []
            for t in window:
                flatten.extend(t)
            windowed_data.append((flatten, labels[i - window_size]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(embed, self.input_placeholder), shape=[-1,self.config.n_window_features*self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier_initializer([self.config.n_window_features*self.config.embed_size, self.config.hidden_size]), dtype=tf.float32, name="W")
        b1 = tf.Variable(tf.zeros([1, self.config.hidden_size]), dtype=tf.float32, name="b1")
        a1 = tf.matmul(x, W)+b1
        a1 = tf.nn.relu(a1)
        a1 = tf.nn.dropout(a1, self.dropout_placeholder)
        b2 = tf.Variable(tf.zeros([1, self.config.n_classes]), dtype=tf.float32, name="b2")
        U = tf.Variable(xavier_initializer([self.config.hidden_size, self.config.n_classes]), dtype=tf.float32, name="U")
        a2 = tf.matmul(a1, U)+b2
