...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            W_z = tf.get_variable("W_z", shape=[self.input_size, self.state_size], dtype=tf.float32, initializer=initializer)
            U_z = tf.get_variable("U_z", shape=[self.state_size, self.state_size], dtype=tf.float32, initializer=initializer)
            b_z = tf.get_variable("b_z", shape=[self.state_size], dtype=tf.float32, initializer=initializer)

            W_r = tf.get_variable("W_r", shape=[self.input_size, self.state_size], dtype=tf.float32, initializer=initializer)
            U_r = tf.get_variable("U_r", shape=[self.state_size, self.state_size], dtype=tf.float32, initializer=initializer)
            b_r = tf.get_variable("b_r", shape=[self.state_size], dtype=tf.float32, initializer=initializer)

            W_o = tf.get_variable("W_o", shape=[self.input_size, self.state_size], dtype=tf.float32, initializer=initializer)
            U_o = tf.get_variable("U_o", shape=[self.state_size, self.state_size], dtype=tf.float32, initializer=initializer)
            b_o = tf.get_variable("b_o", shape=[self.state_size], dtype=tf.float32, initializer=initializer)

            z_t = tf.sigmoid(tf.matmul(inputs, W_z) + tf.matmul(state, U_z) + tf.reshape(b_z, [1, self.state_size]))
            r_t = tf.sigmoid(tf.matmul(inputs, W_r) + tf.matmul(state, U_r) + tf.reshape(b_r, [1, self.state_size]))
            o_t = tf.tanh(tf.matmul(inputs, W_o) + tf.matmul(r_t * state, U_o) + tf.reshape(b_o, [1, self.state_size]))
            new_state = o_t + z_t * (state - o_t)
